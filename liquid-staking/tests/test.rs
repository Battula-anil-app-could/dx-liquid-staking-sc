...
use dharitri_sc_scenario::{num_bigint, rust_biguint, DebugApi};
...
fn liquid_staking_remove_liquidity_test() {
    ...
    sc_setup.check_user_moax_balance(&first_user, 0u64);
}
...
fn liquid_staking_claim_rewards_and_withdraw_test() {
    ...
    sc_setup.check_user_balance(&first_user, LS_TOKEN_ID, 10u64);
    sc_setup.check_user_moax_balance_denominated(&first_user, 91232876712328767122u128);
}
...
fn liquid_staking_multiple_operations() {
    ...
    sc_setup.b_mock.set_block_nonce(10u64);
    sc_setup.recompute_token_reserve(&first_user);
    sc_setup.check_user_moax_balance_denominated(
    ...
    sc_setup.delegate_rewards(&first_user);
    sc_setup.check_user_moax_balance_denominated(sc_setup.sc_wrapper.address_ref(), 0u128);
    ...
    sc_setup.b_mock.set_block_epoch(60u64);
    sc_setup.check_user_moax_balance(&first_user, 20u64);
    sc_setup.unbond_tokens(&first_user, UNSTAKE_TOKEN_ID, 1);

    let ls_value = sc_setup.get_ls_value_for_position(1u64);
    let initial_moax_balance = exp18_128(20u64);
    let ls_token_balance_in_moax = 70 * ls_value;
    let rounding_offset = 6u128;
    sc_setup.check_user_moax_balance_denominated(
        &first_user,
        initial_moax_balance + ls_token_balance_in_moax + rounding_offset,
    );
}
...
fn liquid_staking_multiple_withdraw_test() {
    ...
    sc_setup.unbond_tokens(&first_user, UNSTAKE_TOKEN_ID, 1);
    sc_setup.check_user_balance(&first_user, LS_TOKEN_ID, 30u64);
    sc_setup.check_user_moax_balance(&first_user, 70);
    sc_setup.check_user_balance(&second_user, LS_TOKEN_ID, 20u64);
    sc_setup.check_user_moax_balance(&second_user, 60);
    sc_setup.check_user_balance(&third_user, LS_TOKEN_ID, 20u64);
    sc_setup.check_user_moax_balance(&third_user, 60);
