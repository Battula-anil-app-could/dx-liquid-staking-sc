...
use dharitri_sc::{
    ...
    types::{Address, BigUint, DctLocalRole},
};
...
pub static DCT_ROLES: &[DctLocalRole] = &[
    DctLocalRole::Mint,
    DctLocalRole::Burn,
    ...
    LiquidStakingContractObjBuilder: 'static + Copy + Fn() -> liquid_staking::ContractObj<DebugApi>,
{
    pub fn new(sc_builder: LiquidStakingContractObjBuilder) -> Self {
        ...
        b_mock.set_dct_local_roles(sc_wrapper.address_ref(), LS_TOKEN_ID, DCT_ROLES);
        b_mock.set_dct_local_roles(sc_wrapper.address_ref(), UNSTAKE_TOKEN_ID, SFT_ROLES);
