...
pub const MIN_MOAX_TO_DELEGATE: u64 = 1_000_000_000_000_000_000;
    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn add_liquidity(&self) {
        ...
        require!(payment >= MIN_MOAX_TO_DELEGATE, ERROR_BAD_PAYMENT_AMOUNT);
        ...
        self.delegation_proxy_obj()
            ...
            .with_moax_transfer(payment.clone())
            .async_call()
    ...
    fn add_liquidity_callback(
        ...
        match result {
            ...
            ManagedAsyncCallResult::Err(_) => {
                self.send().direct_moax(&caller, &staked_tokens);
    ...
    fn remove_liquidity(&self) {
        ...
        require!(
            storage_cache.ls_token_id.is_valid_dct_identifier(),
        ...
        let moax_to_unstake = self.pool_remove_liquidity(&payment.amount, &mut storage_cache);
        require!(
            moax_to_unstake >= MIN_MOAX_TO_DELEGATE,
            ERROR_INSUFFICIENT_UNSTAKE_AMOUNT
        );
        self.burn_ls_token(&payment.amount);

        let delegation_contract = self.get_delegation_contract_for_undelegate(&moax_to_unstake);
        let gas_for_async_call = self.get_gas_for_async_call();

        self.delegation_proxy_obj()
            .contract(delegation_contract.clone())
            .undelegate(moax_to_unstake.clone())
            .with_gas_limit(gas_for_async_call)
            .async_call()
    ...
    fn remove_liquidity_callback(
        ...
        match result {
            ManagedAsyncCallResult::Ok(()) => {
                ...
                self.delegation_contract_data(&delegation_contract)
                    .update(|contract_data| {
                        contract_data.total_staked_from_ls_contract -= &moax_to_unstake;
                        contract_data.total_unstaked_from_ls_contract += &moax_to_unstake;
                    });
                self.unstake_token_supply()
                    .update(|x| *x += &moax_to_unstake);

                let virtual_position = UnstakeTokenAttributes {
                    delegation_contract,
                    unstake_epoch: current_epoch,
                    unstake_amount: moax_to_unstake,
                    unbond_epoch,
                };
            ...
            ManagedAsyncCallResult::Err(_) => {
                let ls_token_amount = self.pool_add_liquidity(&moax_to_unstake, &mut storage_cache);
                let user_payment = self.mint_ls_token(ls_token_amount);
                self.send().direct_dct(
                    &caller,
                    &user_payment.token_identifier,
    ...
    fn unbond_tokens(&self) {
        ...
        if delegation_contract_data.total_unbonded_from_ls_contract >= unstake_amount {
            ...
            storage_cache.total_withdrawn_moax -= &unstake_amount;
            self.unstake_token_supply()
                .update(|x| *x -= &unstake_amount);
            self.burn_unstake_tokens(payment.token_nonce);
            self.send().direct_moax(&caller, &unstake_amount)
        } else {
            let gas_for_async_call = self.get_gas_for_async_call();
            self.delegation_proxy_obj()
                .contract(delegation_contract.clone())
    ...
    fn withdraw_tokens_callback(
        ...
        match result {
            ManagedAsyncCallResult::Ok(()) => {
                let withdraw_amount = self.call_value().moax_value().clone_value();
                let mut storage_cache = StorageCache::new(self);
                ...
                if withdraw_amount > 0u64 {
                    ...
                    storage_cache.total_withdrawn_moax += &withdraw_amount;
                }
                let delegation_contract_data = delegation_contract_mapper.get();
                if delegation_contract_data.total_unbonded_from_ls_contract >= unstake_token_amount
                {
                    ...
                    storage_cache.total_withdrawn_moax -= &unstake_token_amount;
                    self.unstake_token_supply()
                        .update(|x| *x -= &unstake_token_amount);
                    self.burn_unstake_tokens(unstake_token_nonce);
                    self.send().direct_moax(&caller, &unstake_token_amount);
                } else {
    ...
    fn recompute_token_reserve(&self) {
        ...
        let current_moax_balance = self
            .blockchain()
            .get_sc_balance(&MoaxOrDctTokenIdentifier::moax(), 0);
        if current_moax_balance
            > &storage_cache.total_withdrawn_moax + &claim_status.starting_token_reserve
        {
            let rewards = &current_moax_balance
                - &storage_cache.total_withdrawn_moax
                - &claim_status.starting_token_reserve;
            storage_cache.rewards_reserve += rewards;
        }

        if storage_cache.rewards_reserve >= MIN_MOAX_TO_DELEGATE {
    ...
    fn delegate_rewards(&self) {
        ...
        self.delegation_proxy_obj()
            ...
            .delegate()
            .with_gas_limit(gas_for_async_call)
            .with_moax_transfer(rewards_reserve.clone())
            .async_call()
            .with_callback(
    ...
    fn delegate_rewards_callback(
        ...
        match result {
            ManagedAsyncCallResult::Ok(()) => {
                ...
                storage_cache.virtual_moax_reserve += &staked_tokens;
    ...
    fn get_ls_value_for_position(&self, ls_token_amount: BigUint) -> BigUint {
        let storage_cache = StorageCache::new(self);
        self.get_moax_amount(&ls_token_amount, &storage_cache)
    }
